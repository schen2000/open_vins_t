...
class path_pub():
    def __init__(self):
        ...
        self.out_parent_frame_id = rospy.get_param("~out_parent_frame_id", 'global')
        self.child_frame_id = rospy.get_param("~child_frame_id", '/imu')
        self.in_topic_name = rospy.get_param("~in_topic_name", '/my_tf')
...
#        self.my_pose = rospy.Subscriber('/tf', TFMessage, self.tfcallback)
        self.my_pose = rospy.Subscriber(self.in_topic_name, TransformStamped, self.tfcallback2)
    ...
    def tfcallback2(self, msg):
        if msg.header.frame_id==self.parent_frame_id and msg.child_frame_id==self.child_frame_id:
            self.pose = msg.transform
            self.check = 1
    ...
    while 1:
        try:
            if path_pub_class.check == 1:
                ...
                pose.header.frame_id = path_pub_class.out_parent_frame_id
                pose.header.stamp = rospy.Time.now()
                path_pub_class.my_path.header.frame_id=path_pub_class.out_parent_frame_id
